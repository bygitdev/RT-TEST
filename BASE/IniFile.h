...
#include <tchar.h>
        ...
        // the callback, function pointer needs to be passed to "Ini::ParseDNTString".
        ...
        public:		
            ...
            Ini(LPCTSTR lpPathName); // Construct with a given file name
            virtual ~Ini();
...
/*  
	...
	Ini* pConfig = new Ini("C:\\KOSES\\SEQ\\BuzzerConfig.ini");

	m_config[TYPE_ERR].nLampGOnOff		= pConfig->GetInt("ERR_STATE", "G_LAMP", 0);
	m_config[TYPE_ERR].nLampYOnOff		= pConfig->GetInt("ERR_STATE", "Y_LAMP", 0);
	m_config[TYPE_ERR].nLampROnOff		= pConfig->GetInt("ERR_STATE", "R_LAMP", 0);
	m_config[TYPE_ERR].nBuzOnOff		= pConfig->GetInt("ERR_STATE", "BUZZER", 0);
	m_config[TYPE_ERR].nMelody			= pConfig->GetInt("ERR_STATE", "MELODY", 0);
